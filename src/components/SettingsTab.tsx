    ...
    return (
        <div className="flex flex-col h-full bg-[#F5EFE5] rounded-[18px] border border-[#EADFCF] overflow-hidden animate-fade-in w-full max-w-5xl">
            <div className="bg-white border-b border-[#EADFCF] p-[20px_26px_16px] flex items-center justify-between">
                <div>
                    <h2 className="font-bricolage font-extrabold tracking-[-0.5px] text-[23px] text-[#2A1D14]">{t("settings.title")}</h2>
                    <div className="text-[12.5px] text-[#7A6A5C] mt-1">{t("settings.subtitle")}</div>
                </div>
                <button 
                ...
                >
                    {savingPricing ? <Loader2 className="h-5 w-5 animate-spin" /> : t("settings.saveChanges")}
                </button>
            </div>
            ...
            <div className="flex-1 p-[24px_30px] grid grid-cols-1 lg:grid-cols-[1.25fr_1fr] gap-[22px] overflow-y-auto">
                ...
                <div className="bg-white border border-[#EADFCF] rounded-[18px] flex flex-col overflow-hidden">
                    <div className="p-[18px_24px_14px] border-b border-[#F1EBE3] flex items-baseline gap-[12px]">
                        <span className="flex-1 font-bricolage font-extrabold tracking-[-0.4px] text-[18px] text-[#2A1D14]">{t("settings.basePricing")}</span>
                        <span className="text-[12.5px] text-[#7A6A5C]">{garments.length} {t("settings.garments")}</span>
                    </div>
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-0">
                        ...
                        <div className="flex items-center gap-[12px] p-[14px_24px] cursor-pointer hover:bg-[#FBF7F0] transition-colors">
                            ...
                            <span className="flex-1 text-[14px] font-extrabold text-[#8A5A1E]">{t("settings.addGarmentType")}</span>
                        </div>
                    </div>
                ...
                <div className="flex flex-col gap-[22px]">
                    <div className="bg-white border border-[#EADFCF] rounded-[18px] p-[20px_22px]">
                        <div className="font-bricolage font-extrabold tracking-[-0.4px] text-[18px] text-[#2A1D14]">{t("dash.dailyStitchCapacity")}</div>
                        <div className="text-[13px] text-[#7A6A5C] mt-[5px]">{t("settings.capacityHint")}</div>
                        <div className="flex items-center gap-[14px] mt-[16px]">
                            <div className="flex-1 bg-[#FBF7F0] border-2 border-[#C8912F] rounded-[12px] p-[13px_16px] flex items-baseline gap-[9px]">
                                ...
                                <span className="text-[13px] text-[#7A6A5C]">{t("settings.setsPerDay")}</span>
                            </div>
                    ...
                    <div className="flex-1 bg-white border border-[#EADFCF] rounded-[18px] p-[20px_22px]">
                        <div className="flex items-baseline gap-[10px]">
                            <span className="flex-1 font-bricolage font-extrabold tracking-[-0.4px] text-[18px] text-[#2A1D14]">{t("settings.next7Days")}</span>
                            <span className="text-[12.5px] text-[#7A6A5C]">{t("settings.bookedAgainst")} 50</span>
                        </div>
                        <div className="flex flex-col gap-[13px] mt-[18px]">
                            ...
                            <div>
                                <div className="flex justify-between text-[13px] font-bold text-[#A6947F] mb-[6px]">
                                    ...
                                    <span>{lang === 'ta' ? 'விடுமுறை' : 'Closed'}</span>
                                </div>
