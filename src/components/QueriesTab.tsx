        ...
        <div className="flex w-full h-full overflow-hidden bg-white animate-fade-in">
            ...
            <div className="w-full md:w-[400px] bg-white border-r border-[#EADFCF] flex flex-col overflow-hidden shrink-0">
                <div className="p-[20px_22px_14px] border-b border-[#EADFCF]">
                    <div className="font-bricolage font-extrabold tracking-[-0.5px] text-[23px] text-[#2A1D14]">{t("queries.title")}</div>
                    <div className="text-[12.5px] text-[#7A6A5C] mt-1">{queryingUsers.length} {t("queries.waiting")} &middot; 128 {t("queries.thisMonth")}</div>
                    <div className="flex gap-[7px] mt-[14px]">
                        <span className="px-[13px] py-[8px] rounded-full bg-[#2A1D14] text-[#F7EEDC] text-[12.5px] font-bold cursor-pointer">{t("queries.all")} {queryingUsers.length}</span>
                        <span className="px-[13px] py-[8px] rounded-full bg-[#FBF7F0] border border-[#E5D9C7] text-[#5E4A38] text-[12.5px] font-semibold cursor-pointer">{t("queries.unread")} {queryingUsers.filter(u => (u.queryCount || 0) > 0).length}</span>
                    </div>
                </div>
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="text-[12.5px] text-[#5E4A38] mt-[3px] leading-[1.4] line-clamp-2">
                                        {lastMsg?.text || (t("queries.noHistory"))}
                                    </div>
                ...
                {selectedUser ? (
                    <>
                        ...
                        <div className="flex-1 p-[22px_26px] flex flex-col gap-[12px] overflow-y-auto">
                            <div className="text-center text-[11.5px] text-[#A6947F] font-extrabold tracking-[0.8px] my-4">{t("queries.history")}</div>
                        ...
                        <div className="bg-white p-4 border-t border-[#EADFCF]">
                            <div className="flex gap-2">
                                <input 
                                    type="text" 
                                    placeholder={t("queries.typeReply")} 
